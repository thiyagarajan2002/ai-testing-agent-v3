...
public class ReportManager {
    ...
    private void writeHtml(ExecutionResult r) throws Exception {
        String rows = r.steps.stream().map(s ->
                "<tr><td>" + esc(s.action) + "</td><td>" + (s.passed ? "PASS" : "FAIL") +
                "</td><td>" + s.durationMs + "</td><td>" + esc(s.details) + "</td></tr>")
                .collect(Collectors.joining());
        String html = """
        <!doctype html><html><head><meta charset="UTF-8"><title>AI Testing Agent Report</title>
        <style>body{font-family:Arial;margin:30px;background:#fafafa}table{border-collapse:collapse;width:100%%}th,td{border:1px solid #ccc;padding:8px;text-align:left;vertical-align:top}th{background:#eee}.pass{font-weight:bold}</style>
        </head><body><h1>AI Testing Agent Report</h1>
        <p><b>Test:</b> %s</p><p><b>Status:</b> %s</p>
        <p><b>Steps:</b> %d &nbsp; <b>Passed:</b> %d &nbsp; <b>Failed:</b> %d</p>
        <table><tr><th>Action</th><th>Status</th><th>Duration(ms)</th><th>Details / Input &amp; Output</th></tr>%s</table>
        <h2>Failure Analysis</h2><pre>%s</pre></body></html>
        """.formatted(esc(r.testName), r.passed ? "PASS" : "FAIL", r.steps.size(),
                r.steps.stream().filter(s -> s.passed).count(), r.steps.stream().filter(s -> !s.passed).count(), rows, esc(r.failureAnalysis));
    ...
    private void writeCsv(ExecutionResult r) throws Exception {
        try (BufferedWriter w = Files.newBufferedWriter(dir.resolve("report.csv"))) {
            ...
            for (var s : r.steps) w.write(csv(r.testName) + "," + csv(s.action) + "," + (s.passed ? "PASS" : "FAIL") + "," + s.durationMs + "," + csv(s.details) + "\n");
        }
    }
    ...
    private void writePdf(ExecutionResult r) throws Exception {
        Files.writeString(dir.resolve("report.pdf"), "AI Testing Agent Report\n\nTest: " + r.testName +
                ...
                s.action + " | " + (s.passed ? "PASS" : "FAIL") + " | " + s.durationMs + "ms | " + s.details).collect(Collectors.joining("\n")));
    }
