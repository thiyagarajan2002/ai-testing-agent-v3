...
public final class Main {
 private static final String VERSION="3.21.0"; private Main(){}
 public static void main(String[] a){
  ...
  try{
   ...
   try(RunLogManager logs=RunLogManager.start(c.reportsDir(),area,command)){
    logs.log("Command: " + String.join(" ", args));
    int code=run(args);
    logs.log("Exit code: " + code);
    if(code!=0)System.exit(code);
   }
  }catch(AgentExecutionException e){System.err.println("Agent error ["+e.category()+"]: "+e.getMessage());System.exit(2);}
  catch(Exception e){System.err.println("Agent error [INFRASTRUCTURE]: "+e.getMessage());System.exit(2);}
 }
 ...
 private static String logArea(String[] a){if(a.length==0)return "terminal";if("plan".equalsIgnoreCase(a[0])||"suite".equalsIgnoreCase(a[0])||"data-driven".equalsIgnoreCase(a[0])||("validate".equalsIgnoreCase(a[0])&&a.length>1&&"plan".equalsIgnoreCase(a[1])))return "api";if("interactive".equalsIgnoreCase(a[0]))return "ui";return "terminal";}
 ...
 private static int parallelism(String[] a,int fallback){for(int i=0;i<a.length-1;i++)if("--parallelism".equalsIgnoreCase(a[i])){try{return Integer.parseInt(a[i+1]);}catch(NumberFormatException e){throw new AgentExecutionException(AgentExecutionException.Category.CONFIGURATION,"--parallelism must be an integer: "+a[i+1],e);}}return fallback;}
