...
public class Main {
    ...
    private static void run(String[] args) throws Exception {
        ...
        EnvironmentManager environments = new EnvironmentManager(mapper);
        EnvironmentProfile profile = env == null ? null : environments.load(env, Path.of("."));

        if (args.length >= 2 && "plan".equalsIgnoreCase(args[0])) {
            executeFile(args[1], mapper, runner, reports, profile, environments);
            return;
        }
        if (args.length >= 2 && "suite".equalsIgnoreCase(args[0])) {
            executeSuite(args[1], mapper, runner, config, profile, environments);
            return;
        }
        System.out.println("=== AI Testing Agent v" + VERSION + " ===");
        System.out.println("Commands: plan <file> [--env <name>] | suite <file> [--env <name>] | interactive | exit");
    ...
    private static void executeFile(String file, ObjectMapper mapper, AgentRunner runner,
                                    ReportManager reports, EnvironmentProfile profile,
                                    EnvironmentManager environments) throws Exception {
        ...
        TestPlan plan;
        try {
            plan = mapper.readValue(Files.readString(path), TestPlan.class);
        } catch (Exception e) {
            throw new AgentExecutionException(AgentExecutionException.Category.PLAN_VALIDATION,
                    "Invalid test plan JSON: " + path, e);
        }
        ...
        System.out.println("Environment: " + (profile == null ? "default" : profile.name));
    ...
    private static void executeSuite(String file, ObjectMapper mapper, AgentRunner runner,
                                     Config config, EnvironmentProfile profile,
                                     EnvironmentManager environments) throws Exception {
        ...
        try {
            ...
            throw new AgentExecutionException(AgentExecutionException.Category.SUITE_VALIDATION,
                    "Invalid suite JSON: " + suitePath, e);
        }
        Path dir = suitePath.getParent() == null ? Path.of(".").toAbsolutePath() : suitePath.getParent();
        SuiteExecutionEngine engine = new SuiteExecutionEngine(mapper, runner, config.parallelism(), environments, profile);
        SuiteExecutionResult result = engine.execute(suite, dir);
        Path reports = Path.of(config.reportsDir(), "suite").toAbsolutePath().normalize();
        new SuiteReportManager(reports).writeAll(result);
        ...
        System.out.println("Suite reports: " + reports);
        if (!result.passed()) System.exit(1);
    }
                ...
                try {
                    ...
                    if (!result.passed()) {
                        try {
                            ...
                            System.err.println("AI failure analysis unavailable: " + e.getMessage());
                        }
                    }
                    reports.writeAll(result);
                    ...
                    System.err.println("Agent error [" + categoryOf(e) + "]: " + e.getMessage());
                }
            }
        }
    }

    private static AgentExecutionException.Category categoryOf(Exception e) {
