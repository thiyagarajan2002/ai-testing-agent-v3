...
public class Main {
    public static void main(String[] args) throws Exception {
        ...
        AgentRunner runner=new AgentRunner(llm,mapper); ReportManager reports=new ReportManager();
        if(args.length>=2 && "plan".equalsIgnoreCase(args[0])) { executeFile(args[1],mapper,runner,reports); return; }
        System.out.println("=== AI Testing Agent v3.0.0 ===");
        System.out.println("Commands: plan <file> | interactive | exit");
    ...
    private static void executeFile(String file,ObjectMapper mapper,AgentRunner runner,ReportManager reports)throws Exception{
        Path path=Path.of(file); if(!Files.exists(path))throw new IllegalArgumentException("Plan file not found: "+file);
        TestPlan plan=mapper.readValue(Files.readString(path),TestPlan.class); ExecutionResult result=runner.execute(plan);
        if(!result.passed()){try{result=runner.analyzeFailure(plan,result);}catch(Exception e){System.err.println("AI failure analysis unavailable: "+e.getMessage());}}
        reports.writeAll(result); System.out.println("Result: "+(result.passed()?"PASSED":"FAILED")); System.out.println("Reports written to ./reports");
    }
