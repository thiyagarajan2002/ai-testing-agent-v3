...
public class UiExecutor {
        ...
        try (Playwright playwright = Playwright.create()) {
            ...
            for (TestStep step : plan.steps) {
                ...
                try {
                    ...
                    result.passed = false; captureFailure(page, result.steps.size());
                    result.steps.add(new ExecutionResult.StepResult(step.action, false, e.toString(), System.currentTimeMillis() - start)); break;
                }
            }
            browser.close();
    ...
    private void captureFailure(Page page, int index) { try { Path dir=Path.of("reports","screenshots"); Files.createDirectories(dir); page.screenshot(new Page.ScreenshotOptions().setPath(dir.resolve("failure-" + index + ".png"))); } catch(Exception ignored) {} }
