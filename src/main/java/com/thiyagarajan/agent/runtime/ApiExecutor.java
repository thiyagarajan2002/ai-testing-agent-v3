...
public class ApiExecutor {
    ...
    public ExecutionResult execute(TestPlan plan){
        ...
        for(TestStep step:plan.steps){
            long start=System.currentTimeMillis(); int attempts=0; Exception last=null; boolean done=false;
            int maxAttempts=step.retryCount>=0?step.retryCount+1:1;
            while(!done && attempts<maxAttempts){
                ...
                try{
                    ...
                    String details="HTTP "+response.statusCode()+"; attempts="+attempts+"; durationMs="+duration;
                    if(step.save!=null)for(var e:step.save.entrySet()){Object value=response.jsonPath().get(e.getValue());if(value==null)throw new AssertionError("JSON extraction failed: "+e.getValue());variables.put(e.getKey(),String.valueOf(value));details+="; saved="+e.getKey();}
                    details+="; response="+abbreviate(response.asString(),1000);
                    if(ok){result.steps.add(new ExecutionResult.StepResult(step.action,true,details,duration)); done=true;}
                    else if(attempts>=maxAttempts){result.steps.add(new ExecutionResult.StepResult(step.action,false,details,duration));result.passed=false;done=true;}
                }catch(Exception e){last=e;if(attempts>=maxAttempts){result.passed=false;result.steps.add(new ExecutionResult.StepResult(step.action,false,"attempts="+attempts+"; "+e,duration(start)));done=true;}}
            }
            if(last!=null && !done){result.passed=false;break;}
            if(!result.passed)break;
        } return result;
    }
    private long duration(long start){return System.currentTimeMillis()-start;}
